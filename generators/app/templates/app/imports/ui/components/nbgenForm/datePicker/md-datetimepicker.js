...
import pickerTemplate from './md-datetimepicker.html';
...
(function (moment) {
  ...
  angular.module(moduleName, ['ngMaterial', 'nbgenUtilsUi'])
    ...
    .directive('mdcDatetimePicker', ['$tmvUiData','$timeout',
      function ($tmvUiData,$timeout) {
        ...
        return {
          ...
          link: function (scope, element, attrs, ngModel) {
            ...
            element.on('focus click', function (e) {
              e.preventDefault();
              // check first if it's readonly mode
              if (element.attr('mdc-read-only') === 'true') {
                return; // mo modification
              }
              ...
              $tmvUiData.formDialog({
                ...
                .finally(function() {
                    $timeout(function() {
                      element.blur();
                      // check if this input element has md-input-container as parent
                      // and remove focused class from it
                      var parent = element.parent();
                      if (parent.length === 1 && parent[0].nodeName === 'MD-INPUT-CONTAINER') {
                        parent.removeClass('md-input-focused');
                      }
                    })
                })
              ;
  ...
  var PluginController = function ($scope, mdPanelRef, mdcDatetimePickerDefaultLocale) {
  ...
  PluginController.$inject = ['$scope', 'mdPanelRef', 'mdcDatetimePickerDefaultLocale'];
  PluginController.prototype = {
    ...
    hide: function (okBtn) {
      if (okBtn) {
        this._closeDialog({action: 'ok', data: this.currentDate});
      } else {
    ...
    _closeDialog: function(reason) {
      this._mdPanelRef && this._mdPanelRef.close(reason).then(() => {
        this._mdPanelRef.destroy();
      })
    }
  };
      ...
      function () {
        ...
        return {
          ...
          link: function (scope, element, attrs) {
            ...
            var componentRoot = document.querySelector('.tmv-date-picker-container.dtp');
            var exec = function () {
            ...
            scope.setTime = function (val) {
              ...
              if (!minuteMode) {
                ...
                picker.currentView = VIEW_STATES.MINUTE;  // change to minuteMode after picking hour; modified by tmv
              } else {
